            ...
            if (self.type(props) == 'string') {
                switch (props) {
                    case 'translateX':
                        ...
                        return style[transform].replace(/[matrix\(\)]/i, '').split(',')[4].trim();
                    case 'translateY':
                        if (style[transform] == 'none') return 0;
                        return style[transform].replace(/[matrix\(\)]/i, '').split(',')[5].trim();
                    default:
                        return style[props];
    ...
    init: function(reverse) {
        var self = this,
            ...
            ready = function() {
                ...
                xhr.open('GET', 'https://www.tigerbrokers.com/api/v1/wx_jsapi?type=tiger&url=' + encodeURIComponent(location.href.replace(/#.*$/, '')), true);
                xhr.onreadystatechange = function() {
