...
(function ($, undefined) {
    ...
    if (typeof TigerBridge != 'undefined') {

        var deadDay = (1229).toString();
        var delayItems = deadDay.length;
        var $delay = $('.delaytime').find('.delay');
        for(var i=0; i<delayItems; i++) {
            $('<div class="item">'+ deadDay.charAt(i) +'</div>').appendTo($delay);
        }
        $('<div class="item">天</div>').appendTo($delay);

        $('.product').tap(function(e) {
            var tar = e.target,
                $tar = $(e.target),
                tname = tar.className;

            if (/pay/gi.test(tname)) {
                $(document).on('touchstart', pret);
                $paytype.show().find('.value').text('￥' + $tar.attr('data-price'));
                $paywrap.addClass('active');
            }
        })

        $(document).tap(function(e) {
            if (e.target.className == 'paytype') {
                $(document).off('touchstart', pret);
                $paytype.hide();
        ...
        // 选择支付方式
        var typeIndex = 0;
        var items = $('.paytype').find('.type-item');
        items.tap(function(e) {
            items.eq(typeIndex).find('.select').removeClass('active');
            typeIndex = $(this).attr('data-index');
            $(this).find('.select').addClass('active');
        });

        // 确认支付
        $paytype.find('.sure-pay').tap(function(e) {
            console.log('xxx');
        })

        // hide fail window
        $('.pay-fail').find('.close').tap(function() {
            $('.pay-fail').hide();
        });


    } else {
        window.location.href = 'https://www.tigerbrokers.com?from=hkpay';
    }
    ...
    console.log(Boolean(getParam(window.location.href, 'boardid2')));
